...
// allow CORS permission for whiteboard server only
app.use(cors({
  origin: 'http://localhost:9002',
  expose: ['Accept-Ranges']
}));

// middleware to provide access-control to File Server
/*app.use(function *(next){
  var siploCookie = this.cookies.get('siplo_cookie');

  if(this.request.query.id) {
    console.log('############################################');
    //console.log('COOKIE - '+this.cookies.get('_ga'));
    //console.log('Query Params - '+ this.request.query.id);
    console.log('Query Path - ' + this.request.path);
    console.log('############################################');
    yield next;
  } else{
